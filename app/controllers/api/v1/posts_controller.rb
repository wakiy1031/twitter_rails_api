class Api::V1::PostsController < ApplicationController
  ...
  def show
    ...
    render json: { data: post.as_json(include: { images: { only: [:id, :filename, :content_type, :byte_size] } }) }
  ...
  def find_post(post_id)
    ...
    unless post
      render json: { message: '画像登録先の投稿が見つかりませんでした。' }, status: :unprocessable_entity
    end
    post
  end
